...
const OWishlist = () => {
  ...
  useEffect(() => {
    const fetchTournamentData = async () => {
      try {
        const url = `/players/get_wishlist?token=${token}`;
  ...
  console.log('====================================');
  console.log(org[0].tournament.tournament_games[0].id);
  console.log('====================================');
  ...
  <div className="">
    <div className="flex flex-row items-center justify-between w-full mb-10 border-b pb-4 border-gray-300">
      ...
      <div className="lg:w-1/2 w-full ">
        <h1 className="sm:text-3xl text-2xl font-medium title-font text-gray-900">Your Wishlist</h1>
    ...
    <div className="flex flex-wrap -m-4">
      {org.map((card, index) => (
        <Link
        to = {`/organizer/tournament-details/${card.tournament.id}`}
         key={index} className="xl:w-1/4 md:w-1/2 p-4">
          <div className="bg-white rounded-lg shadow-lg">
            <img
              className="h-40 rounded w-full object-cover object-center mb-6"
              src={`http://127.0.0.1:8000/organizer/images/${card.tournament.image}`}
              alt="content"
            />
            ...
            <h3 className="tracking-widest text-xs font-medium title-font pl-3">
              {card.tournament.is_active ? <><p className="text-green-500">Active</p></> : <><p className="text-red-500">Closed</p></>}
            </h3>
<p>Ends on : 24 March 2023</p>
            ...
            <p className="leading-relaxed text-base pl-3">{card.tournament.about}</p>
